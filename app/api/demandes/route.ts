...
export async function POST(request: NextRequest) {
  try {
    ...
    const cookie = (name: string) => request.cookies.get(name)?.value || "";
    const parsed = demandSchema.safeParse({
      ...
      utmSource: form.get("utm_source") || cookie("mycoco_utm_source"),
      utmMedium: form.get("utm_medium") || cookie("mycoco_utm_medium"),
      utmCampaign: form.get("utm_campaign") || cookie("mycoco_utm_campaign"),
    });

    if (!parsed.success || parsed.data.website) {
      return errorRedirect(request, form.get("locale") === "en" ? "en" : "fr", "validation");
    }
