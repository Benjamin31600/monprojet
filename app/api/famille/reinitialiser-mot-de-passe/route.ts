...
export async function POST(request: NextRequest) {
  ...
  if (!token || password.length < 10 || password !== confirmPassword) return NextResponse.redirect(new URL(`/${locale}/reinitialiser-mot-de-passe?token=${encodeURIComponent(token)}&erreur=validation`, request.url), 303);
