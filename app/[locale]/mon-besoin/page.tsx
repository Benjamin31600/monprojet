...
export default async function MyNeedPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="need-page">
    <style>{`
      .need-page{min-height:calc(100vh - 78px);background:linear-gradient(180deg,#eef5ef 0%,#f8f4ea 42%,#f8f4ea 100%)}
      .need-hero{padding:48px 0 34px;border-bottom:1px solid var(--line)}
      .back-link{display:inline-flex;align-items:center;gap:7px;color:var(--green);font-size:.78rem;font-weight:850;margin-bottom:34px}
      .back-link:hover{text-decoration:underline}
      .need-hero .eyebrow{display:block;color:var(--green);font-size:.68rem;font-weight:900;letter-spacing:.16em}
      .need-hero h1{max-width:820px;margin:12px 0 14px;font-size:clamp(2.55rem,5vw,4.7rem);line-height:1;letter-spacing:-.065em}
      .need-hero p{max-width:650px;margin:0;color:var(--ink-2);font-size:1.02rem;line-height:1.65}
      .need-section{padding:42px 0 88px}
      .need-card{width:min(760px,100%);margin:0 auto;padding:30px;background:#fff;border:1px solid var(--line);border-radius:24px;box-shadow:var(--shadow)}
      .need-step{display:grid;grid-template-columns:46px 1fr;gap:15px;align-items:start;padding:4px 0 16px}
      .need-step>span{display:grid;place-items:center;width:42px;height:42px;border-radius:13px;background:var(--sage);color:var(--green);font-size:.68rem;font-weight:900}
      .need-step h2{margin:0;font-size:1.08rem;letter-spacing:-.025em}
      .need-step p{margin:3px 0 0;color:var(--muted);font-size:.75rem;line-height:1.5}
      .need-field{display:block;padding:0 0 28px;margin:0 0 26px;border-bottom:1px solid #edf1ee}
      .need-field>span{display:block;margin:0 0 8px;color:var(--ink-2);font-size:.74rem;font-weight:850}
      .need-field input,.need-field select{width:100%;height:52px;padding:0 14px;border:1px solid var(--line-strong);border-radius:12px;background:#fff;color:var(--ink);outline:0;font-size:.88rem;transition:border-color .15s ease,box-shadow .15s ease}
      .need-field input::placeholder{color:#9aa7a1}
      .need-field input:focus,.need-field select:focus{border-color:var(--green);box-shadow:0 0 0 4px rgba(35,107,88,.09)}
      .need-submit{width:100%;min-height:54px;margin-top:0;font-size:.9rem}
      .need-disclaimer{margin:14px auto 0;max-width:600px;text-align:center;color:var(--muted);font-size:.67rem;line-height:1.55}
      @media(max-width:700px){.need-hero{padding:34px 0 28px}.need-hero h1{font-size:2.55rem}.need-section{padding:28px 0 60px}.need-card{padding:21px 18px;border-radius:20px}.need-step{grid-template-columns:40px 1fr;gap:12px}.need-step>span{width:38px;height:38px}.need-field{padding-bottom:22px;margin-bottom:22px}}
    `}</style>
