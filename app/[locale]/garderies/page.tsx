...
export default async function GarderiesPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<Params> }) {
  ...
  return <main className="search-page">
    ...
    <section className="results-shell"><div className="section-inner">
      {demande === "enregistree" && <div className="matching-success"><div className="matching-success-icon">✓</div><div><strong>{fr ? "Votre demande de matching est enregistrée" : "Your matching request is saved"}</strong><p>{fr ? "MyCoco a bien enregistré votre besoin. Ces critères pourront servir à prioriser les établissements et à construire le futur matching entre votre demande et les solutions disponibles." : "MyCoco has saved your childcare need. These criteria can be used to prioritize providers and build future matching between your request and available solutions."}</p></div><Link href={`/${locale}/mon-besoin`}>{fr ? "Modifier" : "Edit"}</Link></div>}
    ...
    <style>{`.matching-success{display:flex;align-items:center;gap:14px;margin-bottom:18px;padding:16px 18px;background:#edf7ef;border:1px solid #cfe4d3;border-radius:17px}.matching-success-icon{display:grid;place-items:center;flex:0 0 38px;width:38px;height:38px;border-radius:12px;background:var(--green);color:#fff;font-weight:900}.matching-success strong{display:block;color:var(--green);font-size:.82rem}.matching-success p{margin:3px 0 0;color:var(--ink-2);font-size:.7rem;line-height:1.5}.matching-success a{margin-left:auto;color:var(--green);font-size:.7rem;font-weight:900;white-space:nowrap}.need-summary{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:22px;padding:16px 18px;background:#fff;border:1px solid var(--line);border-radius:17px}.need-summary-items{display:flex;flex-wrap:wrap;gap:8px;margin-top:6px}.need-summary-items strong,.need-summary-items span{padding:5px 9px;border-radius:999px;background:var(--sage);color:var(--ink-2);font-size:.69rem;font-weight:800}.need-summary-items strong{background:var(--green);color:#fff}.need-edit{color:var(--green);font-size:.72rem;font-weight:900;white-space:nowrap}@media(max-width:700px){.matching-success{align-items:flex-start}.matching-success a{margin-left:0}.need-summary{align-items:flex-start;flex-direction:column}.need-edit{white-space:normal}}`}</style>
  </main>;
}
