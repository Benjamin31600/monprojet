...
export async function generateMetadata({
  ...
  const fr = locale === "fr";
  const title = fr
    ? "MyCoco | Garderies, CPE et solutions de garde au Québec"
    : "MyCoco | Daycares, CPEs & childcare in Quebec";
    ...
    <>
      <header className="site-header">
        <div className="site-header-inner">
          <Link className="brand" href={`/${locale}`} aria-label="MyCoco">
            my<span>coco</span>
          </Link>
          <nav aria-label={locale === "fr" ? "Navigation principale" : "Main navigation"}>
            ...
            <Link className="language-switch" href={`/${other}`}>
              {other.toUpperCase()}
            </Link>
            <Link className="header-cta" href={`/${locale}/garderies`}>
              {d.nav.cta}
            </Link>
          </nav>
        </div>
